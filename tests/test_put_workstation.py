...
def test_put_workstation(client: TestClient):
    response = client.put("workstation/1", json={
        "name": "teste",
        "asdl_vpn": True,
        "regional": True,
        "city_id": 1
...
def test_put_non_existing_workstation(client: TestClient):
    response = client.put("workstation/90", json={
        "name": "teste",
        "asdl_vpn": True,
        "regional": True,
        "city_id": 1
    ...
    assert response.json()[
        "message"] == "O Posto de Trabalho de id = 90 não está cadastrado."
...
def test_put_workstation_without_regional(client: TestClient):
    ...
    assert response.json()[
        "message"] == "Caso o posto de trabalho não seja regional, \
        forneça o a regional à qual ele pertence."
    assert response.status_code == 400
...
def test_put_workstation_with_non_existing_city(client: TestClient):
    ...
    assert response.json()[
        "message"] == "A cidade de id = 50 não está cadastrada."
    assert response.status_code == 400
